...
- The list is centred on the playing track: its number, a play mark, the title (scrolling
  when it does not fit), position / duration and a progress bar. Accented titles render
  (Latin-1). **Tap a row to play that track.**
...
Fonts: VT323 and Orbitron (SIL OFL, `include/OFL-*.txt`) embedded as 1-bit GFX fonts with
Latin-1 coverage, generated by `scripts/gen_gfxfont.py` (`uv run --with pillow`).
